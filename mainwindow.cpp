...
{
    int cx,cy;
    float rad =(-1)* ui->transRotDeg->text().toFloat()*0.0174533;
    float c = cos(rad);
    float s = sin(rad);
    if(ui->graph->recent.size()==0){
        ...
        cx=ui->graph->recent[0].first.toInt();        cy=ui->graph->recent[0].second.toInt();
    }
    for(int i = 0; i < ui->graph->points.size(); i++){
        int x = ui->graph->points[i].first.first;
        int y = ui->graph->points[i].first.second;
        ui->graph->points[i].first.first =c*(x-cx)-s*(y-cy)+cx;
        ui->graph->points[i].first.second =s*(x-cx)+c*(y-cy)+cy;
    }
    ui->graph->repaint();
}
...
void MainWindow::reflectAboutLine(){
    ...
    int vSize=ui->graph->recent.size();
    int j;
    for(j=0;j<vSize-1;j++){
        drawLineDDA(ui->graph->points[j].first.first,ui->graph->points[j].first.second,ui->graph->points[j+1].first.first,ui->graph->points[j+1].first.second);
        drawLineDDA(ui->graph->recent[j].first.toInt(),ui->graph->recent[j].second.toInt(),ui->graph->recent[j+1].first.toInt(),ui->graph->recent[j+1].second.toInt());
    }
    drawLineDDA(ui->graph->points[0].first.first,ui->graph->points[0].first.second,ui->graph->points[j].first.first,ui->graph->points[j].first.second);
    drawLineDDA(ui->graph->recent[0].first.toInt(),ui->graph->recent[0].second.toInt(),ui->graph->recent[j].first.toInt(),ui->graph->recent[j].second.toInt());
